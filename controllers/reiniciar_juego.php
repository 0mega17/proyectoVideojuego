...
try {
    $sqlDelete = "
DELETE ct FROM casillas_tablas ct
JOIN tablas t ON t.id = ct.tablas_id
WHERE t.codigos_codigo = :codigoSala
";
    $delete = $mysql->getConexion()->prepare($sqlDelete);
    $delete->bindParam(":codigoSala", $codigoSala, PDO::PARAM_STR);
    $delete->execute();
